...
trait MsiData
{
    protected function msiData($request)
    {
        ...
        // search
        if (!empty($search)) {
            $columns = $this->msiValidArray(explode(',', $column), 'msiColumn');
        ...
        // relation
        if (!empty($relation)) {
            $relations = $this->msiValidArray(explode(',', $relation), 'msiMethod');
            $query->with($relations);
        }

        // order
        if (!empty($order)) {
            $orders = $this->msiValidArray(explode(',', $order), 'msiColumn');
